...
//*** This Class for Add New Team To The Competition ***//
public class AddTeamPage {
	...
	protected static Stage addTeamStage;
	protected static AnchorPane addTeamPane;
	protected static Scene addTeamScene;
	...
	protected static TextFieldStyler teamNameTextField, rankTextField;

	public AddTeamPage(Competition comp) {
		addTeamPane = new AnchorPane();
		addTeamScene = new Scene(addTeamPane, WIDTH, HEIGHT);
		addTeamStage = new Stage();
		addTeamStage.setScene(addTeamScene);
		addTeamStage.setOnCloseRequest(e -> addTeamStage.close());
		addTeamScene.getStylesheets().add(getClass().getResource("competitionDetail.css").toExternalForm());

		createBackground();
		createLabels();
		createButtons(comp);

		teamNameTextField = new TextFieldStyler("Team name", 327, 32, 350, 213);
		rankTextField = new TextFieldStyler("Rank", 327, 32, 350, 280);
		addTeamPane.getChildren().addAll(teamNameTextField, rankTextField);
	}
	...
	public void createBackground() {
		...
		addTeamPane.setBackground(new Background(background));
	}
	...
	private void createLabel(String text, double fontSize, double layoutX, double layoutY) {
		...
		addTeamPane.getChildren().add(label);
	}

	// Competition Name Label and TextField
	private void createLabels() {
		createLabel("Add Team", 20, 350, 160);
		createLabel("Team name", 15, 350, 207);
		createLabel("Rank", 15, 350, 275);
	}

	public void createButtons(Competition comp) {
		createAddAnotherTeamButton(comp);
		createCreateButton(comp);
		createCancelButton();
	}

	public void createAddAnotherTeamButton(Competition comp) {
		...
		button.setOnAction(e -> {
			...
			if (teamNameTextField.getText().isEmpty()) {
				...
				if (rankTextField.getText().isEmpty()) {
					Team newTeam = Controller.database.createTeam(teamNameTextField.getText(), -1, comp);
					competitionDetailsPageTeam = new CompetitionDetailsPageTeam(comp);
					competitionDetailsPageTeam.teamParticipant.add(newTeam);
					CompetitionPage.update();
					HomePage.update();
					teamNameTextField.clear();
					rankTextField.clear();
				} else if (!isNumeric(rankTextField.getText()) || (Integer.parseInt(rankTextField.getText()) <= 0
						&& Integer.parseInt(rankTextField.getText()) != -1)) {
					AlertBox.display("ERROR", "Please Enter a Valid Rank");
				} else {
					Team newTeam = Controller.database.createTeam(teamNameTextField.getText(),
							Integer.parseInt(rankTextField.getText()), comp);
					competitionDetailsPageTeam = new CompetitionDetailsPageTeam(comp);
					competitionDetailsPageTeam.teamParticipant.add(newTeam);
					CompetitionPage.update();
					HomePage.update();
		...
		addTeamPane.getChildren().add(button);
	}
